...
/**
 * This project's own lock key, and it MUST NOT match a sibling's.
 *
 * The key is global to a Postgres instance, not to a database. Two projects
 * sharing a key would serialise against each other for no reason — and if they
 * ever shared a server, one project's suite would silently block on another's
 * for up to the timeout below and then report a hung run. The sibling uses
 * 8_140_255_301; this is deliberately a different number.
 */
const SUITE_LOCK_KEY = 5_193_744_812;
