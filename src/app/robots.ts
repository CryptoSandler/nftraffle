 ...
 *
 * This product is not launched. Until it is, every deployment (production
 * included) should be reachable by a person with the URL and invisible to a
 * search index: a half-built raffle platform indexed under a working name
 * nobody has bought the domain for is a first impression nobody chose.
 *
 * This is one of THREE independent mechanisms, deliberately, because they fail
 * in different places and no single one covers a crawler's whole path:
 *
 ...
 * link. Layers 2 and 3 are what make the answer "do not index" rather than "do
 * not look", and they only work on requests robots.txt did not stop.
 *
