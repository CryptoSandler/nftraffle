...
/**
 * The only methods this product needs, and the only ones this proxy will ever
 * forward.
 *
 * Signing:
 * - getLatestBlockhash: every transaction needs a recent blockhash.
 * - sendTransaction: submit the signed transaction.
 * - getSignatureStatuses: show "confirming…" before the server verifies.
 *
 * Reading back what a creator just deployed, which is what makes the platform
 * fee enforceable (spec §5.3): the creator assembles the candy machine
 * transaction, so the creator can also assemble one without our `solFixedFee`
 * guard. Reading the account back is what catches that.
...
/**
 * A JSON-RPC call for one of the whitelisted methods comfortably fits in a few
 * hundred bytes; the largest, `sendTransaction`, carries a base64-encoded
 ...
 *
 * The cap is on the REQUEST. A DAS response for a large collection is far
 * bigger than this and is unaffected — nothing here bounds what comes back.
 */
