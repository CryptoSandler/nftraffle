/**
 * Which Solana cluster this deployment is really talking to, in words.
 *
 * Its own module because the answers are decided in the browser and belong in a
 * test: a component that reads `window.location` cannot be asserted about from
 * Node, and these functions can.
 *
 * WHO CALLS THIS: `classifyEndpoints` is called server-side by every page that
 * renders a signing surface, and its ANSWER — never the URL — is passed down.
 * `paymentSafety` and `isLocalHostname` are called in the browser by the
 * signing components.
 */
 ...
 *
 * **This gate covers more than money here.** In the sibling project the only
 * thing a wrong cluster could ruin was a payment, which is bad and recoverable
 * by the operator. This product also asks a creator to sign transactions that
 * create PERMANENT ON-CHAIN STATE — a Core collection, a candy machine. A
 * collection minted on devnet because the proxy was pointed there is not
 * refundable and not undoable, so the launch flow is gated on exactly the same
 * answer as the payment flow (CLAUDE.md, "Showing the network before a
 * signature").
 *
...
export function paymentSafety(input: {
  ...
  if (proxyCluster === "unknown") {
    return {
      ok: false,
      message:
        "This deployment's Solana connection could not be identified as mainnet, so signing is " +
  ...
  if (proxyCluster !== signingChain) {
    return {
      ok: false,
      message:
        `This deployment settles on ${clusterLabel(proxyCluster)}, and your wallet ` +
  ...
  if (signingChain !== "solana:mainnet") {
    return {
      ok: false,
      message:
        `Your wallet would be asked to sign on ${clusterLabel(signingChain)}. This deployment ` +
        "settles on Solana mainnet, so anything signed here could never be credited, and " +
        "signing is turned off on this screen.",
    };
  }
  ...
  if (localOrigin) {
    return {
      ok: false,
      message:
        "This page is served from a development machine. Anything signed here would move real " +
        "SOL on Solana mainnet, so signing is turned off on this screen.",
    };
  }
