 ...
 *
 * `pg` rather than an ORM: every statement in this project is parameterised by
 * hand, and that is a deliberate constraint rather than a stopgap — nothing is
 * ever string-interpolated into SQL. The guarantees this product leans on are
 * ultimately constraints Postgres enforces: one signature claims one payment
 * (`consumed_signatures.signature` PRIMARY KEY), one ticket number exists once
 * per raffle (`tickets` PRIMARY KEY), one asset escrows once
 * (`raffles.escrow_signature` UNIQUE). Those promises are only global when
 * every request talks to the same database rather than a per-process store.
 */
