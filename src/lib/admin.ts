 ...
 * Admin access.
 *
 * Adapted from the sibling projects, which were shaped by three findings and
 * keep their answers here:
 *
 ...
 *
 *     **What that does NOT buy, stated because it is easy to assume it does:**
 *     there is no way to revoke somebody ELSE's session. `revokeAdminSession`
 *     revokes the id in the calling browser's own cookie — revocation is
 *     self-service only. Rotating `ADMIN_TOKEN` to a new value does not kill
 *     live sessions either, because a session row stores a label and never the
 *     secret. An operator whose cookie leaked has two real options: CLEAR
 *     `ADMIN_TOKEN`, which does kill every live session (`resolveAdminSession`
 *     gates on `adminConfigured`) at the cost of taking the whole surface down,
 *     or psql. A revoke-any-session surface is the missing piece; it is named
 ...
 * cookie minted while the token was set can still get into.
 *
 * **This surface matters more here than it did in the sibling.** There, /admin
 * filed stray payments. Here it holds the payout queue — the one place where an
 * operator action has a consequence outside the database, on somebody else's
 * property sitting in escrow.
 *
 * WHO CALLS THIS: `POST /api/admin/session` signs in and `DELETE` on the same
 * route signs out. `authenticateAdmin` is called by `admin-guard.ts` and by
 * nothing else; `adminSessionLabel` is called by the `/admin` server components.
 */
 ...
 * shares (see `clientIp` in `client-ip.ts`, and `identify` in
 ...
 * (`client-ip.ts`), so this function succeeds and hands back the one
